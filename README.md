...
The project compares a recurrent encoder-decoder baseline against an attention-based architecture, performs hyperparameter optimization using Weights & Biases sweeps, evaluates models using sequence-level and character-level metrics, visualizes learned attention alignments, and provides an interactive Streamlit application for inference.
...
These results show that allowing the decoder to dynamically attend to encoder states improves character alignment and transliteration quality compared with compressing the complete source sequence into a single fixed-size hidden representation.
...
Instead of relying only on the final encoder state, the attention model computes a weighted combination of all encoder outputs at every decoding step.
...
### Attention Seq2Seq Search Space
...
The final attention model uses:
...
Training additionally uses gradient clipping, learning-rate scheduling, label smoothing for the final attention model, early stopping, and best-checkpoint selection based on validation word accuracy.

## Sweep Results

### Attention Seq2Seq Hyperparameter Sweep

![Attention Sweep](assets/attention_sweep.png)

The sweep compares attention mechanism, dropout, embedding dimension, and hidden dimension against validation word accuracy.

### Vanilla Seq2Seq Hyperparameter Sweep

![Vanilla Sweep](assets/vanilla_sweep.png)

The baseline sweep explores recurrent cell type, dropout, embedding dimension, hidden dimension, and network depth.
...
Heatmaps can be generated from test-set examples using:
...
- Bayesian hyperparameter optimization with Weights & Biases
